...
export default function Staff() {
    ...
    const fetchStaffs = (query) => {
        StaffService.GetStaffs(query)
            .then(res => {
    ...
    const deleteData = () => {
        if (selected) {
            StaffService.DeleteStaff(selected._id)
                .then(res => {
                    if (res) {
                    ...
                    {staffs?.map((e, i) => (
                        <tr key={i}>
                            ...
                            <td className="flex gap-4 justify-end">
                                <IconButton color="yellow-400" onClick={() => router.push(`/staff/${e._id}`)}>
